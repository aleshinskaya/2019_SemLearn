...
srun -p med -J sub-021 --mem=8000 -t 12:00:00 -u --pty tcsh -l
...
set object_names_list = ('obj1' 'obj2' 'obj3' 'obj7' 'obj8' 'obj9' 'obj4' 'obj5' 'obj6' 'obj10' 'obj11' 'obj12')


#load modules 
module load matlab afni fmriprep singularity gsl bio3 freesurfer/6.0.0 perl

if ( -e $fmriPrepDir'/input'/$subjID'/ses-2' ) then
   echo 'Directory for ses-2 exists'
   mv $fmriPrepDir'/input'/$subjID'/ses-2' $fmriPrepDir'/'$subjID'/'
...
sbatch -p high -t 24:00:00 --mem=16000 run_reconall.sh $subjID $rootDir $sessionID
...
# arg1 = surfaces directory for that subject, eg surfaces/sub-021/; arg2 = filename, arg3= epifile to resample to with full path
...
if ( -e $fmriPrepDir'/'$subjID'/' ) then
	mkdir $fmriPrepDir'/input'/$subjID'/ses-2/'
    mv $fmriPrepDir'/'$subjID'/'* $fmriPrepDir'/input'/$subjID'/ses-2/'
